...
namespace game_framework
{
	...
	void Cabin::SetPassengerPosition(vector<Passenger*> passengerOnCabin)
	{
		if (movingDirection == "left" || movingDirection == "right")
		{
			for (unsigned i = 0; i < passengerOnCabin.size(); i++)
			{
				passengerOnCabin[i]->SetXY(leftTopX + PASSENGERPOSITIONX_RIGHTLEFT[i], leftTopY + PASSENGERPOSITIONY_RIGHTLEFT[i]);
			}
		}
		else if (movingDirection == "down" || movingDirection == "up")
		{
			for (unsigned i = 0; i < passengerOnCabin.size(); i++)
			{
				passengerOnCabin[i]->SetXY(leftTopX + PASSENGERPOSITIONX_UPDOWN[i], leftTopY + PASSENGERPOSITIONY_UPDOWN[i]);
			}
		}
	}
	...
	void Cabin::PassengerGetOn(Passenger* p)
	{
		passengerNum++;
		passengerOnCabinList.push_back(p);
		//passengerNum = passengerOnCabinList.size();
		//SetPassengerPosition();
	...
	int Cabin::PassengerGetOut(vector<Passenger*>& passengerOnCabin, int nowStation)
	{
		outPeople = 0;

		for (unsigned i = passengerOnCabin.size() - 1; i >= 0; i--)
		{
			if (i > 6)
			{
				break;
			}
			else if (nowStation == passengerOnCabin[i]->GetFinalStation())
			{
				delete passengerOnCabin[i];
				passengerOnCabin.erase(passengerOnCabin.begin() + i);
				outPeople++;
			}
			passengerNum = (int)passengerOnCabin.size();
		}
		return 0;
	}
