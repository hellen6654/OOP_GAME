...
class Line
{
    public:
        ...
        bool IsClickedStationStart(int, int, vector<Station>, int);			//檢查是否點到車站 有就回傳ture 如果沒有則回傳false
        bool IsClickedStationEnd(int, int, vector<Station>, int, int);			//檢查是否點到車站 有就回傳ture 如果沒有則回傳false
        ...
        void ShowIconBitmap(int);
        void ShowRailway(vector<Station>, int);							//劃出線路
    private:
