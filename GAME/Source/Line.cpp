...
{
Line::Line()
    : clickedStartStationNum(-1), clickedEndStationNum(-1), lineColor{0, 0, 0}, isCanBeClicked(false)
{}
...
bool Line::IsMouseClickedLineColorBMP(int mouseX, int mouseY)
{
    if (mouseX > lineColorBMP.Left() && mouseX < lineColorBMP.Left() + lineColorBMP.Width() &&
            mouseY > lineColorBMP.Top() && mouseY < lineColorBMP.Top() + lineColorBMP.Height() && isCanBeClicked)
        return true;
...
bool Line::IsIconCanBeClicked()
{
    return isCanBeClicked;
}

void Line::SetIsCanbeClicked(bool b)
{
    isCanBeClicked = b;
}
