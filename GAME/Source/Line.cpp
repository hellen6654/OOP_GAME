...
{
Line::Line()
    : clickedStartStationNum(-1), clickedEndStationNum(-1), lineColor{ 0, 0, 0 }, lineState(0)
{}
...
bool Line::IsClickedStationStart(int x, int y, vector<Station> stationList, int currentStation)
{
    for (int i = 0; i < currentStation; i++)
        if (x >= stationList[i].GetX() && x <= stationList[i].GetX() + 25 && y >= stationList[i].GetY() && y <= stationList[i].GetY() + 25)
            return true;
...
bool Line::IsClickedStationEnd(int x, int y, vector<Station> stationList, int currentStation, int start)
{
    for (int i = 0; i < currentStation; i++)
        if (x >= stationList[i].GetX() && x <= stationList[i].GetX() + 25 && y >= stationList[i].GetY() && y <= stationList[i].GetY() + 25 && i != start)
            return true;
...
void Line::LoadBitmap()
{
    ...
    else if (lineColor[0] == 131 && lineColor[1] == 131 && lineColor[2] == 131)
        lineColorBMP.LoadBitmap(".\\RES\\color\\gray.bmp", RGB(255, 255, 255));
...
void Line::ShowIconBitmap(int x)
{
    if (x == 1)
    {
        lineColorBMP.SetTopLeft(285, 570);
